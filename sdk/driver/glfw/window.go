...
import (
	...
	"github.com/wrzfeijianshen/fyne2/sdk/painter/gl"
...
func (w *window) screenSize(canvasSize fyne.Size) (int, int) {
	return sdk.ScaleInt(w.canvas, canvasSize.Width), sdk.ScaleInt(w.canvas, canvasSize.Height)
}
...
func (w *window) Resize(size fyne.Size) {
	...
	if w.fixedSize || !w.visible { // fixed size ignores future `resized` and if not visible we may not get the event
		w.width, w.height = sdk.ScaleInt(w.canvas, size.Width), sdk.ScaleInt(w.canvas, size.Height)
	}
	w.viewLock.Unlock()
...
func (w *window) fitContent() {
	...
	if w.fixedSize {
		w.width = sdk.ScaleInt(w.canvas, w.Canvas().Size().Width)
		w.height = sdk.ScaleInt(w.canvas, w.Canvas().Size().Height)
...
func (w *window) resized(_ *glfw.Window, width, height int) {
	...
	canvasSize := fyne.NewSize(sdk.UnscaleInt(w.canvas, width), sdk.UnscaleInt(w.canvas, height))
	if !w.fullScreen {
		w.width = sdk.ScaleInt(w.canvas, canvasSize.Width)
		w.height = sdk.ScaleInt(w.canvas, canvasSize.Height)
	}
...
func (w *window) mouseMoved(viewport *glfw.Window, xpos float64, ypos float64) {
	w.mousePos = fyne.NewPos(sdk.UnscaleInt(w.canvas, int(xpos)), sdk.UnscaleInt(w.canvas, int(ypos)))
...
func (w *window) rescaleOnMain() {
	...
	if w.fullScreen {
		...
		scaledFull := fyne.NewSize(
			sdk.UnscaleInt(w.canvas, w.width),
			sdk.UnscaleInt(w.canvas, w.height))
